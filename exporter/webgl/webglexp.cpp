...
// Round numbers near zero to zero.  This help reduce WebGL file size.
inline float
round(float f)
{
...
class WebGLClassDesc:public ClassDesc {
public:
...
static WebGLClassDesc WebGLDesc;

ClassDesc* GetWebGLDesc() { return &WebGLDesc; }

////////////////////////////////////////////////////////////////////////
// WebGL Export implementation
////////////////////////////////////////////////////////////////////////
...
WebGLExport::OutputNodeTransform(INode* node, int level)
{
	...
	if (isIdentity) {
		...
		if (ang != 0.0f && ang != -0.0f) {
			Indent(level);
			// WebGL angle convention is opposite of MAX convention,
...
WebGLExport::OutputNormals(Mesh& mesh, int level)
{
	...
	if (MeshIsAllOneSmoothingGroup(mesh)) {
		...
		// WebGL Browsers compute normals automatically in this case.
		return NULL;
	}
...
BOOL
WebGLExport::HasTexture(INode* node)
{
	TextureDesc* td = GetMatTex(node);
	if (!td)
		return FALSE;
...
WebGLExport::GetMatTex(INode* node)
{
	...
	// We only handle bitmap textures in WebGL
...
WebGLExport::WebGLOutCube(INode* node, Object *obj, int level)
{
	...
	Indent(level+1);
	Point3 p = Point3(0.0f,0.0f,height/2.0f);
	// WebGL cubes grow from the middle, MAX grows from z=0
	fprintf(mStream, _T("Translation { translation %s }\n"), point(p));
	Indent(level+1);
...
WebGLExport::OutputTopLevelLight(INode* node, LightObject *light)
{
	...
	if (id == Class_ID(OMNI_LIGHT_CLASS_ID, 0))
		...
		WebGLOutTopDirectLight(node, light);
...
BOOL
WebGLExport::WebGLOutInline(WebGLInsObject* obj, int level)
{
	Indent(level);
...
BOOL
WebGLExport::WebGLOutLOD(INode *node, LODObject* obj, int level)
{
	...
	for(i = 0; i < numLod; i++) {
		...
		WebGLOutNode(node, parent, level+1, TRUE, FALSE);
	}
...
// lights, cameras and WebGL primitives
BOOL
...
// Returns TRUE for object that we want a WebGL node to occur
// in the file.  
BOOL
WebGLExport::isWebGLObject(INode * node, Object *obj, INode* parent)
{
	if (!obj)
		return FALSE;
...
// Write the WebGL for a single object.
void
WebGLExport::WebGLOutObject(INode* node, INode* parent, Object* obj, int level)
{
	...
	// First check for WebGL primitives and other special objects
	if (WebGLOutSpecial(node, parent, obj, level)) {
		return;
	}
...
void
WebGLExport::WriteLinearKeys(INode* node,
...
Control *
WebGLExport::GetLightColorControl(INode* node)
{
	if (!IsLight(node))
		return NULL;
	Object* obj = node->EvalWorldState(mStart).obj;
...
void
WebGLExport::WebGLOutControllers(INode* node, int level)
{
	Control *pc, *rc, *sc, *vc, *lc;
	int npk = 0, nrk = 0, nsk = 0, nvk = 0, nlk = 0;

	if (mType != Export_ThreeJS)
		return;
...
WebGLExport::WebGLOutFileInfo()
{
	...
	Indent(1);
	fprintf(mStream, _T("Info { string \"Produced by 3D Studio MAX WebGL 1.0/2.0 exporter, Version %.5g, Revision %.5g\" }\n"), vernum, betanum);
						...
						BOOL lastChild)
{
	...
	if (obj && isWebGL) {
		if (!IsLODObject(obj)) {
			...
			WebGLOutControllers(node, level+1);
		}
		// Output the data for the object at this node
		WebGLOutObject(node, parent, obj, level);
	}

	// Now output the children
	for(int i=0; i < numChildren; i++) {
		WebGLOutNode(node->GetChildNode(i), node, level+1,
					FALSE, i == numChildren - 1);
	}
	
	if (node->IsRootNode() || (obj && (isWebGLObject(node, obj, parent)))) {
		if (node->IsRootNode())
...
//		mLodList = mLodList->AddNode(node);
	...
	if (obj) {
		...
		if ((id == Class_ID(SIMPLE_CAM_CLASS_ID, 0) ||
			...
			WebGLOutTopLevelCamera(1, node, FALSE);
	}
...
void
WebGLExport::ComputeWorldBoundBox(INode* node, ViewExp* vpt)
{
	if (!node) return;
	Object* obj = node->EvalWorldState(mStart).obj;
	Class_ID id;
...
BOOL 
WebGLExport::ObjectIsLODRef(INode* node)
{
...
void
WebGLExport::GetCameras(INode *inode, Tab<INode*> *camList,
...
ISpinnerControl* WebGLExport::tformSpin = NULL;
ISpinnerControl* WebGLExport::coordSpin = NULL;
ISpinnerControl* WebGLExport::flipbookSpin = NULL;
...
SampleRatesDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam) 
{
	...
	exp = DLGetWindowLongPtr<WebGLExport *>(hDlg);
	switch (msg) {
	case WM_INITDIALOG: {
...
static INT_PTR CALLBACK
WebGLExportDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam) 
{
	TCHAR text[MAX_PATH];
	WebGLExport *exp;
	ExportType type;
	...
	case WM_INITDIALOG: {
		...
		GetAppData(exp->mIp, OUTPUT_LANG_ID, _T("WebGL"), text, MAX_PATH);
		type = Export_ThreeJS;
	...
	case WM_COMMAND:
		...
		case IDC_EXP_HELP: {
			...
			_tcscat(helpFile, _T("\\webglout.hlp"));
...
int
WebGLExport::DoExport(const TCHAR *filename, ExpInterface *ei, Interface *i, BOOL suppressPrompts, DWORD options) 
{
	mIp = i;
	...
	if (suppressPrompts)
		initializeDefaults();
	else if (!DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_WEBGLEXP), 
						GetActiveWindow(), WebGLExportDlgProc,
						(LPARAM) this))
		...
		if (this->GetFlipBook()) {
			...
			for (int frame = 0; frame < numFrames; frame++, mStart += sampleRate) {
				if (mStart > lastFrame)
					break;
				WebGL2Export webgl2;
				int val = webgl2.DoFBExport(filename, i, this, frame, mStart);
				if (!val) return val;
			}
			return TRUE;
		}
#endif
		WebGL2Export webgl2;
		int val = webgl2.DoExport(filename, i, this);
		return val;
	}
...
void
WebGLExport::initializeDefaults() {
	TCHAR text[MAX_PATH];
...
//	float          mCycleInterval; // Length of animation in seconds
	...
	mZUp = FALSE;        // Z axis if true, Y axis otherwise
	mDigits = 3;     // Digits of precision on output
	mCoordInterp = FALSE;// Generate coordinate interpolators
	mPolygonType = OUTPUT_TRIANGLES;   // 0 triangles, 1 quads, 2 ngons
#ifdef _LEC_
...
int
WebGLExport::ExtCount() {
	return 1;
}

// The exension supported
const TCHAR *
WebGLExport::Ext(int n) {
	switch(n) {
	case 0:
		return _T("html");
	}
...
const TCHAR *
WebGLExport::LongDesc() {
	return _T("Autodesk WebGL");
}
   
const TCHAR *
WebGLExport::ShortDesc() {
	return _T("WebGL");
}

const TCHAR *
WebGLExport::AuthorName() {
	return _T("dave arendash");
}

const TCHAR *
WebGLExport::CopyrightMessage() {
	return _T("Copyright 2012, Autodesk, Inc.");
}

const TCHAR *
WebGLExport::OtherMessage1() {
	return _T("");
}

const TCHAR *
WebGLExport::OtherMessage2() {
	return _T("");
}
...
void
WebGLExport::ShowAbout(HWND hWnd) {
}
...
void 
WebGLExport::GenerateUniqueNodeNames(INode* node)
{
   if (!node) return;
