...
void
WebGL2Export::StartNode(INode* node, int level, BOOL outputName)
{
...
//		"parameters": { "color": 6710886, "wireframe": true }
	...
	fprintf(mStream, _T("\"diffuseColor\": %d\n"), c);
    Indent(level+1);
    ...
    fprintf(mStream, _T("\"specularColor\": %d,\n"), c);
    ...
    if (si > 0.0f) {
        ...
        fprintf(mStream, _T("\"emissiveColor\": %d,\n"), p);
    }
    Indent(level);
    fprintf(mStream, _T("}\n"));
...
WebGL2Export::WebGLOutCamera(INode* node, Object* obj, int level)
{
    ...
    fprintf(mStream, _T("\"%s\": {\n"), mNodes.GetNodeName(node));
    Indent(level+1);
    fprintf(mStream, _T("\"type\": \"perpective\",\n"));
    Indent(level+1);
...
WebGL2Export::WebGLOutObject(INode* node, INode* parent, Object* obj, int level,
                           BOOL mirrored, ClassToFind targetClass)
{
 // need to get a valid obj ptr
	...
	{
        if (targetClass == OBJECTS && isTriMesh && obj->IsRenderable())
		{
            Indent(level);
            fprintf(mStream, _T("\"%s\": {\n"), mNodes.GetNodeName(node));
        }
        ...
        // Output the material
        if (targetClass == MATERIALS && isTriMesh && obj->IsRenderable())  // if not trimesh, needs no matl
    ...
    if (mCoordInterp && isTriMesh && ObjIsAnimated(obj) && !instance)
        WebGLOutCoordinateInterpolator(node, obj, level, mirrored);
}
...
/* test
void
WebGL2Export::WebGLAnchorHeader(INode* node, MrBlueObject* obj,
                              WebGL_TriggerType trigType, BOOL fromParent,
                              int level)
{
    TSTR desc;
    WebGL_Action action = obj->GetAction();
    TCHAR* webglObjName = NULL;
    webglObjName = WebGLParent(node);
    if (!webglObjName)
        return;
    ...
    if (action == HyperLinkJump || action == MrBlueMessage ||
        ...
        case Animate: {
            ...
            for(int i=0; i < size; i++) {
                ...
                AddAnimRoute(webglObjName, ob->name.data(), node, top);
                AddCameraAnimRoutes(webglObjName, node, top);
            }
            break; }
         default:
            assert(FALSE);
                                 ...
                                 INode* top)
{
    for(int i = 0; i < top->NumberOfChildren(); i++) {
        ...
        if (sid == CAMERA_CLASS_ID)
            AddAnimRoute(webglObjName, mNodes.GetNodeName(child),
                fromNode, child);
        AddCameraAnimRoutes(webglObjName, fromNode, child);
    }
}
...
/* test
BOOL
WebGL2Export::WebGLOutMrBlue(INode* node, INode* parent, MrBlueObject* obj,
                          int* level, BOOL fromParent)
{
    ...
    if (obj->GetMouseEnabled()) {
        MrBlueObject* mbo = (MrBlueObject*) obj;
        Indent(*level);
        WebGLAnchorHeader(node, obj, MouseClick, fromParent, *level);
        (*level)++;
        hadHeader = TRUE;
    }
    ...
    if (obj->GetProxDistEnabled()) {
        ...
        WebGLAnchorHeader(node, obj, DistProximity, fromParent, *level);
        Indent(*level+1);
    ...
    if (obj->GetBBoxEnabled() && !fromParent) {
        ...
        if (!hadHeader)
            fprintf(mStream, _T("DEF %s "), name);
        WebGLAnchorHeader(node, obj, BoundingBox, fromParent, *level);
        Indent(*level+1);
    ...
    if (obj->GetLosEnabled()) {
        if (obj->GetLosType() == CanSee) {
            ...
            if (!hadHeader)
                fprintf(mStream, _T("DEF %s "), name);
            WebGLAnchorHeader(node, obj, LineOfSight, fromParent, *level);
            Indent(*level+1);
            fprintf(mStream, _T("distance %s\n"),
...
int
WebGL2Export::StartMrBlueHelpers(INode* node, int level)
{
    ...
    for(int i=0; i<node->NumberOfChildren(); i++) {
        ...
        if (id == Class_ID(MR_BLUE_CLASS_ID1, MR_BLUE_CLASS_ID2)) {
            ...
            if ((mbo->GetAction() == HyperLinkJump ||
                 mbo->GetAction() == SetViewpoint) &&
                mbo->GetMouseEnabled())
                WebGLOutMrBlue(childNode, node, mbo, &level, TRUE);
        }
    }
    return level;
}
...
/* test
void
WebGL2Export::EndMrBlueNode(INode* childNode, int& level, BOOL fromParent)
{
    Object *obj = childNode->EvalWorldState(mStart).obj;
    Class_ID id = obj->ClassID();
    if (id == Class_ID(MR_BLUE_CLASS_ID1, MR_BLUE_CLASS_ID2)) {
...
/* test
void
WebGL2Export::EndMrBlueHelpers(INode* node, int level)
{
    // Check for Mr Blue helper at child nodes
    for(int i=0; i<node->NumberOfChildren(); i++) {
...
WebGL2Export::WebGLOutFileInfo()
{
    ...
    fprintf(mStream, _T("// Produced by 3D Studio MAX WebGL exporter, Version %.5g, Revision %.5g\n"),
        vernum, betanum);
 ...
 // Don't gen code for LOD references, only LOD nodes
    ...
    if (node->IsRootNode())
	{
     ...
     // Compute the world bounding box and a list of timesensors
        ...
        if (mCamera)
            WebGLOutTopLevelCamera(level+ 2, mCamera, TRUE);
        if (mNavInfo)
            WebGLOutTopLevelNavInfo(level+ 2, mNavInfo, TRUE);
        if (mBackground)
            WebGLOutTopLevelBackground(level+ 2, mBackground, TRUE);
        if (mFog)
            WebGLOutTopLevelFog(level+ 2, mFog, TRUE);
...
*/
 ...
 // Anchors need to come first, even though they are child nodes
	...
	{
		WebGLOutObject(node, parent, obj, level+2, mirrored ^ mirror, OBJECTS);
		WebGLOutObject(node, parent, obj, level+2, mirrored ^ mirror, GEOMETRIES);
		WebGLOutObject(node, parent, obj, level+2, mirrored ^ mirror, MATERIALS);
	}
...
WebGL2Export::TraverseNode(INode* node, ClassToFind targetClass)
{
	...
	*/        
        if (targetClass == LIGHTS && IsLight(node))
		{
            mHasLights = TRUE;
        ...
        if (targetClass == CAMERAS && ((id == Class_ID(SIMPLE_CAM_CLASS_ID, 0) ||
             id == Class_ID(LOOKAT_CAM_CLASS_ID, 0))))
            WebGLOutTopLevelCamera(0, node, FALSE);
        
        if (targetClass == BACKGROUNDS && id == BackgroundClassID)
            WebGLOutTopLevelBackground(0, node, FALSE);
        
        if (targetClass == FOGS && id == FogClassID)
            WebGLOutTopLevelFog(0, node, FALSE);
    ...
    for(int i = 0; i < n; i++)
        TraverseNode(node->GetChildNode(i), targetClass);
}
...
WebGL2Export::ScanSceneGraph2()
{
	...
	TraverseNode(node, OBJECTS);
	fprintf(mStream, _T("},\n\n"));

	fprintf(mStream, _T("\"lights\":\n{\n"));
	TraverseNode(node, LIGHTS);
	fprintf(mStream, _T("},\n\n"));

	fprintf(mStream, _T("\"cameras\":\n{\n"));
	TraverseNode(node, CAMERAS);
	fprintf(mStream, _T("}\n\n"));
}
...
#ifdef _LEC_
int
WebGL2Export::DoFBExport(const TCHAR *filename, Interface *i, WebGLExport* exp,
                        int frame, TimeValue time)
{
 ...
 // Write out the WebGL header	
	fprintf(mStream, _T("#WebGL V2.0 utf8\n\n"));
 ...
 // Write out the scene graph
    WebGLOutNode(mIp->GetRootNode(), NULL, -2, FALSE, TRUE, FALSE);
...
WebGL2Export::DoExport(const TCHAR* filename, Interface* i, WebGLExport* exp)
{
 ...
 // Write out the WebGL header and file info
    fprintf(mStream, _T("//WebGL ThreeJS\n\n"));
    WebGLOutFileInfo();
...
/*
void
WebGL2Export::WebGLOutGridHelpers(INode* node)
{
    if (!node) return;
    ...
    int n = node->NumberOfChildren();
    for (int i = 0; i < n; i++) {
        WebGLOutGridHelpers(node->GetChildNode(i));
    }
}
*/
