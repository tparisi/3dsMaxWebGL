 ...
 *<
    ...
    DESCRIPTION:  A WebGL Insert helper implementation
...
class WebGLInsertClassDesc:public ClassDesc {
public:
    int        IsPublic() { return 1; }
    void *        Create(BOOL loading = FALSE) { return new WebGLInsObject; }
    const TCHAR * ClassName() {return _T(GetString(IDS_INLINE_CLASS));}
    SClass_ID     SuperClassID() { return HELPER_CLASS_ID; }
    Class_ID      ClassID() { return Class_ID(WebGL_INS_CLASS_ID1,
                                                    WebGL_INS_CLASS_ID2); }
    const TCHAR*  Category() { return _T("WebGL");  }
};

static WebGLInsertClassDesc vrmlInsertDesc;

ClassDesc* GetWebGLInsertDesc() { return &vrmlInsertDesc; }

// in prim.cpp  - The dll instance handle
extern HINSTANCE hInstance;

ISpinnerControl *WebGLInsObject::sizeSpin = NULL;

HWND WebGLInsObject::hRollup = NULL;

INT_PTR CALLBACK
WebGLInsRollupDialogProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    WebGLInsObject *th = DLGetWindowLongPtr<WebGLInsObject *>( hDlg);  
    if ( !th && message != WM_INITDIALOG ) return FALSE;

    switch ( message ) {
    case WM_INITDIALOG: {
        th = (WebGLInsObject *)lParam;
...
void
WebGLInsObject::BeginEditParams( IObjParam *ip, ULONG flags,Animatable *prev )
{
    iObjParams = ip;
    ...
    if ( !hRollup ) {
        hRollup = ip->AddRollupPage(
            ...
            WebGLInsRollupDialogProc,
            GetString(IDS_WEBGL_INS_TITLE),
            (LPARAM)this );
...
void
WebGLInsObject::EndEditParams( IObjParam *ip, ULONG flags,Animatable *prev)
{
...
WebGLInsObject::WebGLInsObject() : HelperObject() 
{
    radius = 0.0f;
    useSize = TRUE;
...
WebGLInsObject::~WebGLInsObject()
{
}

void
WebGLInsObject::SetSize( float r )
{
    radius = r;
    NotifyDependents(FOREVER, PART_OBJ, REFMSG_CHANGE);
}

IObjParam *WebGLInsObject::iObjParams;

// This is only called if the object MAKES references to other things.
RefResult
WebGLInsObject::NotifyRefChanged(Interval changeInt, RefTargetHandle hTarget, 
...
ObjectState
WebGLInsObject::Eval(TimeValue time){
    return ObjectState(this);
}

Interval
WebGLInsObject::ObjectValidity(TimeValue time)
{
    Interval ivalid;
    ivalid.SetInfinite();
    // UpdateUI(time);
...
void
WebGLInsObject::GetMat(TimeValue t, INode* inode, ViewExp* vpt, Matrix3& tm)
{
...
void
WebGLInsObject::GetLocalBoundBox(TimeValue t, INode* inode, ViewExp* vpt, Box3& box )
{
...
void
WebGLInsObject::GetWorldBoundBox(TimeValue t, INode* inode, ViewExp* vpt, Box3& box )
{
    Matrix3 tm;
...
void
WebGLInsObject::MakeQuad(int *f, int a, int b, int c, int d, int vab, int vbc, int vcd, int vda)
{
...
int
WebGLInsObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags)
{
    if (radius <= 0.0)
        return 0;
    BuildMesh();
...
int
WebGLInsObject::HitTest(TimeValue t, INode *inode, int type, int crossing, int flags, IPoint2 *p, ViewExp *vpt)
{
    HitRegion hitRegion;
    DWORD   savedLimits;
    int res = FALSE;
...
class WebGLInsCreateCallBack : public CreateMouseCallBack {
private:
    IPoint2 sp0;
    Point3 p0;
    WebGLInsObject *vrmlInsObject;

public:
    int proc( ViewExp *vpt,int msg, int point, int flags, IPoint2 m, 
              Matrix3& mat);
    void SetObj(WebGLInsObject *obj) {vrmlInsObject = obj;}

};

         
int
WebGLInsCreateCallBack::proc(ViewExp *vpt,int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{  
    Point3 p1,center;
...
static WebGLInsCreateCallBack vrmlInsCreateCB;

// This method allows MAX to access and call our proc method to 
// handle the user input.
CreateMouseCallBack*
WebGLInsObject::GetCreateMouseCallBack() 
{
...
// IO
#define WebGL_INS_SIZE_CHUNK      0xacb0
#define WebGL_INS_URL_CHUNK    0xacb1
#define WebGL_INS_BBOX_CHUNK     0xacb2

IOResult
WebGLInsObject::Save(ISave *isave)
{
    ULONG written;

    isave->BeginChunk(WebGL_INS_SIZE_CHUNK);
    isave->Write(&radius, sizeof(float), &written);
    isave->EndChunk();

    isave->BeginChunk(WebGL_INS_URL_CHUNK);
#ifdef _UNICODE
    ...
    isave->BeginChunk(WebGL_INS_BBOX_CHUNK);
...
IOResult
WebGLInsObject::Load(ILoad *iload)
{
    ULONG nread;
    IOResult res;
    ...
    while (IO_OK==(res=iload->OpenChunk())) {
        ...
        case WebGL_INS_SIZE_CHUNK:
            iload->Read(&radius, sizeof(float), &nread);
            break;
        case WebGL_INS_URL_CHUNK: {
            char *n;
#ifdef _UNICODE
...
RefTargetHandle
WebGLInsObject::Clone(RemapDir& remap)
{
    WebGLInsObject* vi = new WebGLInsObject();
    vi->radius = radius;
    vi->insURL = insURL;
    vi->useSize = useSize;
