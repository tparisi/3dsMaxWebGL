 ...
 *<
	...
	DESCRIPTION:  WebGL.WRL file export class defs
...
class TextureDesc {
public:
    TextureDesc(BitmapTex* t, TSTR& s, TSTR& u, TSTR& p)
        { tex = t; name = s; url = u; path = p;}
    BitmapTex* tex;
    TSTR       name;
    TSTR       url;
	TSTR		path;
...
class WebGLExport : public SceneExport {
public:
    WebGLExport();
    ~WebGLExport();
    ...
    void GetCameras(INode *inode, Tab<INode*> *camList,
                    ...
                    Tab<INode*> *fogs);
    inline BOOL IsWebGL2() { return mType == Export_ThreeJS; }
...
private:
    ...
    //void WebGLAnchorHeader(MrBlueObject* obj, WebGL_TriggerType type,
...
//    BOOL WebGLOutLOD(INode* node, LODObject* obj, int level);
    //BOOL IsAimTarget(INode* node);
    BOOL WebGLOutTarget(INode* node, int level);
    ...
    BOOL WebGLExport::IsMirror(INode* node);
...
//    INodeList* mLodList;    // List of LOD objects in the scene
    ExportType mType;       // Language to export (ThreeJSL, ...)
